    ...
    <div class="user_list_container">
        ...
        >
            ...
            <template #row-actions="slotProps">
                <router-link :to="{ name: 'AdminRandomItemPlacesDetail', params: { randomItemPlacesId : slotProps.id } }">{{ $t('admin.edit') }}</router-link>
            </template>
        </Datatable>
    </div>
</template>
...
export default defineComponent({
    ...
    methods: {
        loadData() {
            ...
            ApiService.get(urlJoin(import.meta.env.VITE_APP_API_URL+'random_item_places'), params)
                .then((result) => {
                    return result.data;
                })
