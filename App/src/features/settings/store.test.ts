...
describe("Settings store", () => {
    ...
    it("should mark mush actions as hidden", () => {
        store.dispatch("settings/toggleMushActionsVisibility");

        expect(store.getters["settings/hideMushActions"]).toBe(true);
        expect(localStorageService.getItemAsBoolean('hideMushActions')).toBe(true);
    });

    it("should mark mush actions as visible", () => {
        store.dispatch("settings/toggleMushActionsVisibility");
        store.dispatch("settings/toggleMushActionsVisibility");

        expect(store.getters["settings/hideMushActions"]).toBe(false);
        expect(localStorageService.getItemAsBoolean('hideMushActions')).toBe(false);
    });
});
