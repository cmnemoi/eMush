...
const actions: ActionTree<any, any> = {
    ...
    async deleteSkillForPlayer({ dispatch }, { playerId, skill }): Promise<void> {
        await dispatch('admin/displayLoading', null, { root: true });
        try {
            const response: SuccessReponse = await AdminActionsService.deleteSkillForPlayer(playerId, skill);
            await dispatch('toast/openSuccessToast', response.data.detail, { root: true });
        } catch (error) {
            console.error(error);
            await dispatch('error/setError', error, { root: true });
