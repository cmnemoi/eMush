...
const actions: ActionTree<any, any> = {
    ...
    async createStatusForHolderOnDaedalus({ dispatch }, { statusName, holder, daedalus }): Promise<void> {
        await dispatch('admin/displayLoading', null, { root: true });
        try {
            const response: SuccessReponse = await AdminActionsService.createStatusForHolderOnDaedalus(statusName, holder, daedalus);
            await dispatch('toast/openSuccessToast', response.data.detail, { root: true });
        } catch (error) {
            console.error(error);
            await dispatch('error/setError', error, { root: true });
    ...
    async deleteAllSkillsByName({ dispatch }, skillName): Promise<void> {
        await dispatch('admin/displayLoading', null, { root: true });
        try {
            const response: SuccessReponse = await AdminActionsService.deleteAllSkillsByName(skillName);
            await dispatch('toast/openSuccessToast', response.data.detail, { root: true });
        } catch (error) {
            console.error(error);
            await dispatch('error/setError', error, { root: true });
    ...
    async finishProjectForDaedalus({ dispatch }, { projectName, daedalus }): Promise<void> {
        await dispatch('admin/displayLoading', null, { root: true });
        try {
            const response: SuccessReponse = await AdminActionsService.finishProjectForDaedalus(projectName, daedalus);
            await dispatch('toast/openSuccessToast', response.data.detail, { root: true });
        } catch (error) {
            console.error(error);
            await dispatch('error/setError', error, { root: true });
