...
describe("Settings store", () => {
    ...
    it("initializes with default state", () => {
        expect(store.state.settings.settings).toEqual({
    ...
    it("should mark action tabs as visible", () => {
        store.dispatch("settings/toggleActionTabsVisibility");

        expect(store.getters["settings/actionTabs"]).toBe(true);
        expect(localStorageService.getItemAsBoolean('actionTabs')).toBe(true);
    });

    it("should mark action tabs as hidden", () => {
        store.dispatch("settings/toggleActionTabsVisibility");
        store.dispatch("settings/toggleActionTabsVisibility");

        expect(store.getters["settings/actionTabs"]).toBe(false);
        expect(localStorageService.getItemAsBoolean('actionTabs')).toBe(false);
    });

    it("should mark double tap as activated", () => {
        store.dispatch("settings/toggleDoubleTapVisibility");

        expect(store.getters["settings/doubleTap"]).toBe(true);
        expect(localStorageService.getItemAsBoolean('doubleTap')).toBe(true);
    });

    it("should mark double tap as deactivated", () => {
        store.dispatch("settings/toggleDoubleTapVisibility");
        store.dispatch("settings/toggleDoubleTapVisibility");
