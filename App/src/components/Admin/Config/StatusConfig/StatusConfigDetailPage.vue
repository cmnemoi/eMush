...
export default defineComponent({
        ...
        create(): void {
            ...
            GameConfigService.createStatusConfig(newStatusConfig)
                .then((res: StatusConfig | null) => {
                    const newStatusConfigUrl = urlJoin(import.meta.env.VITE_URL + '/config/status-config', String(res?.id));
        ...
        update(): void {
            ...
            GameConfigService.updateStatusConfig(this.statusConfig)
                .then((res: StatusConfig | null) => {
                    ...
                    if (this.statusConfig !== null) {
                        ApiService.get(urlJoin(import.meta.env.VITE_API_URL + 'status_configs', String(this.statusConfig.id), 'modifier_configs'))
                            .then((result) => {
    ...
    beforeMount() {
        ...
        GameConfigService.loadStatusConfig(Number(statusConfigId)).then((res: StatusConfig | null) => {
            this.statusConfig = res;
            ApiService.get(urlJoin(import.meta.env.VITE_API_URL+'status_configs', statusConfigId, 'modifier_configs'))
                .then((result) => {
                    const modifierConfigs : ModifierConfig[] = [];
                    result.data['hydra:member'].forEach((datum: any) => {
                        const currentModifierConfig = (new ModifierConfig()).load(datum);
