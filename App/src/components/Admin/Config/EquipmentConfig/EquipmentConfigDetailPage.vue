...
export default defineComponent({
    ...
    methods: {
        create(): void {
            ...
            GameConfigService.createEquipmentConfig(newEquipmentConfig).then((res: EquipmentConfig | null) => {
                const newEquipmentConfigUrl = urlJoin(import.meta.env.VITE_URL + '/config/equipment-config', String(res?.id));
        ...
        update(): void {
            ...
            GameConfigService.updateEquipmentConfig(this.equipmentConfig)
                .then((res: EquipmentConfig | null) => {
                    ...
                    if (this.equipmentConfig !== null) {
                        ApiService.get(urlJoin(import.meta.env.VITE_API_URL + 'equipment_configs', String(this.equipmentConfig.id), 'actions'))
                            .then((result) => {
                        ...
                        ApiService.get(urlJoin(import.meta.env.VITE_API_URL + 'equipment_configs', String(this.equipmentConfig.id), 'init_statuses'))
                            .then((result) => {
                        ...
                        ApiService.get(urlJoin(import.meta.env.VITE_API_URL + 'equipment_configs', String(this.equipmentConfig.id), 'mechanics'))
                            .then((result) => {
    ...
    beforeMount() {
        ...
        GameConfigService.loadEquipmentConfig(Number(equipmentConfigId)).then((res: EquipmentConfig | null) => {
            this.equipmentConfig = res;
            ApiService.get(urlJoin(import.meta.env.VITE_API_URL+'equipment_configs', equipmentConfigId, 'actions'))
                .then((result) => {
                    const actions : Action[] = [];
                    result.data['hydra:member'].forEach((datum: any) => {
                        const currentAction = (new Action()).load(datum);
            ...
            ApiService.get(urlJoin(import.meta.env.VITE_API_URL+'equipment_configs', equipmentConfigId, 'init_statuses'))
                .then((result) => {
            ...
            ApiService.get(urlJoin(import.meta.env.VITE_API_URL+'equipment_configs', equipmentConfigId, 'mechanics'))
                .then((result) => {
                    const mechanics : Mechanics[] = [];
                    result.data['hydra:member'].forEach((datum: any) => {
                        const currentMechanics = (new Mechanics()).load(datum);
