...
class DaedalusController extends AbstractGameController
{
    ...
    /**
     * Skip current cycle change for all locked up Daedaluses.
     *
     * @OA\Tag (name="Daedalus")
     *
     * @Security (name="Bearer")
     *
     * @Rest\Post(path="/unlock-all-locked-up-daedaluses")
     */
    public function skipCycleChangeForAllLockedUpDaedaluses(): View
    {
        /** @var User $user */
        $user = $this->getUser();
        ...
        foreach ($lockedDaedaluses as $daedalus) {
            $this->daedalusService->skipCycleChange($daedalus);
        }
    ...
    /**
     * Skip current cycle change for locked up Daedalus.
     *
     * @OA\Tag (name="Daedalus")
     *
     * @Security (name="Bearer")
     ...
     */
    public function skipLockedDaedalusCycleChange(Request $request): View
    {
        $daedalusId = $request->get('id');
        ...
        $lockedDaedalus = $this->daedalusService->findById($daedalusId);
        if (!$lockedDaedalus) {
            return $this->view(['error' => 'Daedalus not found'], Response::HTTP_NOT_FOUND);
        }
        if (!$lockedDaedalus->isCycleChange()) {
            return $this->view(['error' => 'Daedalus is not on cycle change'], Response::HTTP_BAD_REQUEST);
        }

        $this->daedalusService->skipCycleChange($lockedDaedalus);
