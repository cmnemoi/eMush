...
class RoomEventService implements RoomEventServiceInterface
{
    ...
    public function handleIncident(Place $room, \DateTime $date): Place
    {
        ...
        //Tremors
        if ($this->randomService->isSuccessful($difficultyConfig->getTremorRate())) {
            $roomEvent = new RoomEvent($room, $date);
            $this->eventDispatcher->dispatch($roomEvent, RoomEvent::TREMOR);
        }

        //Electric Arcs
        if ($this->randomService->isSuccessful($difficultyConfig->getElectricArcRate())) {
            $roomEvent = new RoomEvent($room, $date);
    ...
    public function handleNewFire(Place $room, \DateTime $date): Place
    {
        ...
        if ($fireStatus === null && $this->randomService->isSuccessful($difficultyConfig->getStartingFireRate())) {
            $roomEvent = new RoomEvent($room, $date);
