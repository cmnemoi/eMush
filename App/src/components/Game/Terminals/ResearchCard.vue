<template>
    <div
        :class="['project', { ongoing: project.isLastAdvancedProject }]"
        v-if="project"
    >
        <h3>{{ project.name }}</h3>
        <div class="card">
            <Tippy>
                <img
                    :src="getImgUrl(`researches/${project.key}.png`)"
                    :alt="project.name"
                />
...
.project {
    ...
    margin-right: 6px;
    background: $lightCyan url("/src/assets/images/neroncore_bg.svg") no-repeat
        right bottom;
