<template>
    <div id="comms-panel">
        ...
        <Tippy tag="div" class="cycle-time">
            <img :src="getAssetUrl('comms/calendar.png')"><span>{{ calendar?.dayName }} {{ calendar.day }} - <br />{{ calendar?.cycleName }} {{ calendar.cycle }}</span><span class="mobile">{{ calendar.day }}-{{ calendar.cycle }}</span>
            <template #content>
