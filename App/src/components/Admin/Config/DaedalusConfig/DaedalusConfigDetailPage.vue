...
export default defineComponent({
                    ...
                    .then((res: RandomItemPlaces | null) => {
                        ...
                        GameConfigService.createDaedalusConfig(newDaedalusConfig)
                            .then((res: DaedalusConfig | null) => {
                                const newDaedalusConfigUrl = urlJoin(import.meta.env.VITE_URL + "/config/daedalus-config", String(res?.id));
        ...
        update(): void {
            ...
            GameConfigService.updateDaedalusConfig(this.daedalusConfig)
                .then((res: DaedalusConfig | null) => {
                    ...
                    if (this.daedalusConfig !== null) {
                        ApiService.get(urlJoin(import.meta.env.VITE_API_URL + 'daedalus_configs', String(this.daedalusConfig.id), 'random_item_places'))
                            .then((result) => {                                
                        ...
                        ApiService.get(urlJoin(import.meta.env.VITE_API_URL + 'daedalus_configs', String(this.daedalusConfig.id), 'place_configs?pagination=false'))
                            .then((result) => {
    ...
    beforeMount() {
        ...
        GameConfigService.loadDaedalusConfig(Number(daedalusConfigId)).then((res: DaedalusConfig | null) => {
            this.daedalusConfig = res;
            ApiService.get(urlJoin(import.meta.env.VITE_API_URL+'daedalus_configs', daedalusConfigId, 'random_item_places'))
                .then((result) => {
                    if (this.daedalusConfig instanceof DaedalusConfig) {
                        this.daedalusConfig.randomItemPlaces = (new RandomItemPlaces()).load(result.data);
                    }
                });
            ApiService.get(urlJoin(import.meta.env.VITE_API_URL+'daedalus_configs', daedalusConfigId, 'place_configs?pagination=false'))
                .then((result) => {
                    const placeConfigs : PlaceConfig[] = [];
                    result.data['hydra:member'].forEach((datum: any) => {
                        const currentPlaceConfig = (new PlaceConfig()).load(datum);
