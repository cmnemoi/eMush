...
final class SkillConfigRepository extends ServiceEntityRepository implements SkillConfigRepositoryInterface
{
    ...
    {
        return $skill->isMushSkill() ? $this->findOneMushByNameAndDaedalusOrThrow($skill, $daedalus) : $this->findOneHumanByNameAndDaedalusOrThrow($skill, $daedalus);
    }

    public function findOneHumanByNameAndDaedalusOrThrow(SkillEnum $skill, Daedalus $daedalus): SkillConfig
    {
        $query = <<<'EOD'
        SELECT skill_config.*
        FROM daedalus
    ...
    public function findOneMushByNameAndDaedalusOrThrow(SkillEnum $skill, Daedalus $daedalus): SkillConfig
    {
        $query = <<<'EOD'
        SELECT skill_config.*
        FROM daedalus
        INNER JOIN daedalus_info ON daedalus.id = daedalus_info.daedalus_id
        INNER JOIN game_config_skill_config ON daedalus_info.game_config_id = game_config_skill_config.game_config_id
        INNER JOIN skill_config ON game_config_skill_config.skill_config_id = skill_config.id
        WHERE daedalus_id = :daedalusId
        AND skill_config.name = :skillName
        EOD;
