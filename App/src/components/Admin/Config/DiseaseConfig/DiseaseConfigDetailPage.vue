    ...
    methods: {
        create(): void {
            ...
            GameConfigService.createDiseaseConfig(newDiseaseConfig).then((res: DiseaseConfig | null) => {
                const newDiseaseConfigUrl = urlJoin(import.meta.env.VITE_URL + '/config/disease-config', String(res?.id));
        ...
        update(): void {
            ...
            GameConfigService.updateDiseaseConfig(this.diseaseConfig)
                .then((res: DiseaseConfig | null) => {
                    ...
                    if (this.diseaseConfig !== null){
                        ApiService.get(urlJoin(import.meta.env.VITE_API_URL+'disease_configs', String(this.diseaseConfig.id), 'modifier_configs'))
                            .then((result) => {
    ...
    beforeMount() {
        ...
        GameConfigService.loadDiseaseConfig(Number(diseaseConfigId)).then((res: DiseaseConfig | null) => {
            ...
            if (res instanceof DiseaseConfig) {
                this.diseaseConfig = res;
                ApiService.get(urlJoin(import.meta.env.VITE_API_URL+'disease_configs', diseaseConfigId, 'modifier_configs'))
                    .then((result) => {
                        const modifierConfigs : ModifierConfig[] = [];
                        result.data['hydra:member'].forEach((datum: any) => {
